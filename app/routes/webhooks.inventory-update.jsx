...
async function sendEmail(emailData) {
  try {
    ...
    if (!res.ok) {
      ...
      console.log("✅ Resend accepted email:", resBody.id);
    }
...
// Helper: Get current inventory level from Shopify
...
export async function action({ request }) {
  try {
    ...
    // Load app settings with fallback defaults
    ...
    // Fetch variant + product details from Shopify
    ...
    // Extract clean variantId for OR query
    const variantGid = variant.id; // "gid://shopify/ProductVariant/123"
    const variantIdClean = variantGid?.split('/').pop(); // "123"
    ...
    // ============================================================
    if (available > 0) {
      console.log(`✅ Product is back in stock (qty: ${available})`);

      // ✅ FIX: Match by BOTH inventoryItemId AND variantId
      //         notified filter removed — so previously notified users
      //         also get re-notified when product restocks again
      ...
      console.log(`📧 Found ${subscribers.length} total subscribers for this product`);

      // Filter: only send to those NOT yet notified for this restock cycle
      const toNotify = subscribers.filter(s => !s.notified);
      console.log(`📧 Sending back-in-stock emails to ${toNotify.length} pending subscribers`);
      ...
      for (const sub of toNotify) {
          ...
          <div style="background-color: #f3f4f6; padding: 40px 0; font-family: sans-serif;">
            <table align="center" width="100%" style="max-width: 550px; background-color: #ffffff; border-radius: 24px; overflow: hidden; box-shadow: 0 10px 15px rgba(0,0,0,0.1);">
              <tr><td style="padding: 40px; text-align: center;">
                ...
                <p style="color: #6b7280;">Good news! <strong>${variant.product.title}</strong> is now available at <strong>${shopName}</strong>.</p>
              </td></tr>
        ...
        if (sent) {
          ...
          console.log(`❌ Failed to send email to ${sub.email}`);
        }
      }

      // Also check for price drops among already-notified subscribers
      console.log("🔍 Checking for price drops among all subscribers...");
      for (const sub of subscribers) {
        ...
        // Skip if already handled in toNotify (they got back-in-stock email)
        ...
        const openUrl  = `${APP_URL}api/track-open?id=${sub.id}`;
        const clickUrl = `${APP_URL}api/track-click?id=${sub.id}&target=${encodeURIComponent(productUrl)}`;
        const badge    = getPriceDropBadge(sub.subscribedPrice.toFixed(2), currentPrice.toFixed(2), currency, percentageOff);
          ...
          <div style="background-color: #f3f4f6; padding: 40px 0; font-family: sans-serif;">
            <table align="center" width="100%" style="max-width: 550px; background-color: #ffffff; border-radius: 24px; overflow: hidden; box-shadow: 0 10px 15px rgba(0,0,0,0.1);">
              <tr><td style="padding: 40px; text-align: center;">
                ...
                <p style="color: #6b7280;">The price just dropped on <strong>${shopName}</strong>.</p>
              </td></tr>
              <tr><td style="padding: 0 40px; text-align: center;">
                <div style="background-color: #f9fafb; border-radius: 20px; padding: 30px;">
                  <img src="${productImg}" style="width: 100%; max-width: 250px; border-radius: 12px; margin-bottom: 20px;" alt="${variant.product.title}">
    ...
    // ============================================================
      ...
      // ✅ KEY FIX: Reset notified = false so next restock triggers emails again
      ...
      const adminHtml = `
        <div style="font-family: sans-serif; padding: 30px; background-color: #fffafb; border: 1px solid #fee2e2; border-radius: 16px; max-width: 500px; margin: 20px auto;">
          ...
          <div style="margin-top: 25px; text-align: center;">
            <a href="https://${shop}/admin/products" style="background-color: #111827; color: white; padding: 12px 25px; border-radius: 10px; text-decoration: none; font-weight: bold;">
              Manage Inventory
            </a>
          </div>
        </div>
      ...
      if (sent) {
        console.log("✅ Admin out-of-stock alert sent");
      } else {
