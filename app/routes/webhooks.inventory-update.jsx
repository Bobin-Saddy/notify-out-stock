...
export async function action({ request }) {
  ...
  try {
    ...
    // 1. Updated GraphQL Query to fetch both Variant and Product Images
    ...
    if (!variant) return new Response("No variant", { status: 200 });

    // Image priority: 1. Variant Image, 2. Product Image, 3. Placeholder
    ...
    const productCardHtml = `
      <div style="font-family: Arial, sans-serif; border: 1px solid #eeeeee; padding: 20px; border-radius: 12px; max-width: 450px; background-color: #ffffff;">
        ${imageUrl ? `<img src="${imageUrl}" alt="${productTitle}" style="width: 100%; border-radius: 8px; margin-bottom: 15px; display: block;" />` : ''}
    ...
    if (available <= 0) {
      await sendEmail({
        ...
        html: `<div style="padding: 20px; background-color: #f9f9f9;"><h1>Inventory Alert</h1>${productCardHtml}</div>`
      });
    } 
    
    // --- CASE B: IN STOCK (Customers) ---
    else {
      ...
      if (subscribers.length > 0) {
        for (const sub of subscribers) {
          ...
          await new Promise(r => setTimeout(r, 500)); // Delay for Rate Limit
        }
      }
    }
